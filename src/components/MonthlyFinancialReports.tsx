...
export const MonthlyFinancialReports: React.FC = () => {
  const { 
    receipts, 
    schoolInfo, 
    students, 
  ...
  // Extract unique months from receipts
  ...
  const bankCollected = filteredReceipts
    .filter(r => r.paymentMode === 'Bank Transfer' || r.paymentMode === 'Cheque')
    .reduce((sum, r) => sum + (r.amountPaid || 0), 0);

  // Group by Date for Day-wise Register
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `WisdomSchool_FinancialReport_${reportScope === 'month' ? selectedMonth : 'FullYear'}.csv`);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Controls & Report Header (Hidden on print) */}
      <div className="no-print bg-white p-5 rounded-xl border border-[#E2E8E2] shadow-xs space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-base font-bold text-[#2D312E] flex items-center gap-2">
              <FileSpreadsheet className="w-5 h-5 text-[#4F6D7A]" />
              Monthly School Financial Statement & Audit Register
            </h2>
            <p className="text-xs text-[#6B7280]">
              Admin: {schoolInfo.adminName} • Wisdom Nursery and Primary School, Essur - 603301
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 bg-[#F2F4F2] hover:bg-[#E2E8E2] text-[#2D312E] font-bold rounded-lg text-xs transition-colors border border-[#E2E8E2]"
            >
            ...
            <button
              onClick={handlePrint}
              className="flex items-center gap-1.5 px-4 py-2 bg-[#4F6D7A] hover:bg-[#415A65] text-white font-bold rounded-lg text-xs shadow-xs transition-colors"
            >
        ...
        {/* Filter Toolbar */}
        <div className="flex flex-wrap items-center gap-3 pt-2 border-t border-[#E2E8E2]/70 text-xs">
          <div className="flex items-center gap-2">
