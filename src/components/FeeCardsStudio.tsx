...
export const FeeCardsStudio: React.FC = () => {
  ...
  const selectedTemplate = themeIndex;

  const photoFileInputRef = useRef<HTMLInputElement | null>(null);

  const templates = WEBSITE_THEMES;
    ...
    <div className="space-y-6">
      ...
      <div className="no-print bg-white p-5 rounded-xl border border-[#E2E8E2] shadow-xs space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
          ...
          <div className="pt-3 border-t border-[#E2E8E2]/70">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-bold text-[#2D312E]">Choose Template colours</span>
              <span className="text-[11px] text-[#6B7280]">{template.name}</span>
            </div>
            <div className="grid grid-cols-5 sm:grid-cols-10 gap-2">
              {templates.map((item, index) => (
                <button
                  ...
                  className={`rounded-lg p-1.5 border-2 transition-all ${selectedTemplate === index ? 'border-[#2D312E] ring-2 ring-[#89A894]' : 'border-transparent hover:border-[#CBD5D1]'}`}
                >
                  <span className="block h-7 rounded-md" style={{ background: `linear-gradient(135deg, ${item.primary} 0 60%, ${item.accent} 60%)` }} />
                  <span className="block text-[10px] font-bold text-[#4B5563] mt-1">{index + 1}</span>
                </button>
              ))}
            </div>
          </div>
      ...
      <div className="flex justify-center p-2">
        ...
        {cardType === 'idcard' && (
          <div className="printable-document bg-white border-2 rounded-2xl max-w-md w-full p-5 text-slate-900 shadow-lg relative overflow-hidden" style={{ borderColor: template.primary }}>
            ...
            <div className="text-white -m-5 mb-4 p-4 text-center relative border-b-2" style={{ backgroundColor: template.primary, borderBottomColor: template.accent }}>
              ...
              <div className="mt-2 inline-block text-white text-[10px] font-bold px-3 py-0.5 rounded-full uppercase tracking-widest shadow-2xs" style={{ backgroundColor: template.accent }}>
        ...
        {cardType === 'fee' && (
          <div className="printable-document bg-white border-2 rounded-xl max-w-2xl w-full p-6 text-slate-900 shadow-md relative overflow-hidden" style={{ borderColor: template.primary }}>
            {/* Header with logo */}
