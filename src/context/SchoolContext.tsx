...
import { isDatabaseApiEnabled, loadDatabaseSnapshot, replaceDatabaseResource } from '../utils/databaseApi';
...
export const SchoolProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [themeIndex, setThemeIndexState] = useState(() => {
    const saved = Number(localStorage.getItem('wisdom_theme_index'));
    return Number.isInteger(saved) && saved >= 0 && saved < WEBSITE_THEMES.length ? saved : 0;
  });
  const [databaseHydrated, setDatabaseHydrated] = useState(!isDatabaseApiEnabled);

  const setThemeIndex = (index: number) => {
    if (index >= 0 && index < WEBSITE_THEMES.length) setThemeIndexState(index);
  };

  useEffect(() => {
    if (!isDatabaseApiEnabled) return;
    ...
    loadDatabaseSnapshot()
      ...
      .catch(error => {
        console.warn('Database API unavailable; continuing with localStorage.', error);
      })
      .finally(() => setDatabaseHydrated(true));
  }, []);
  ...
  useEffect(() => {
    localStorage.setItem('wisdom_theme_index', String(themeIndex));
  }, [themeIndex]);

  useEffect(() => {
    localStorage.setItem('wisdom_students', JSON.stringify(students));
    if (databaseHydrated) {
      replaceDatabaseResource('students', students).catch(error => console.warn('Student database sync failed.', error));
    }
  }, [students, databaseHydrated]);

  useEffect(() => {
    localStorage.setItem('wisdom_receipts', JSON.stringify(receipts));
    if (databaseHydrated) {
      replaceDatabaseResource('receipts', receipts).catch(error => console.warn('Receipt database sync failed.', error));
    }
  }, [receipts, databaseHydrated]);

  useEffect(() => {
    localStorage.setItem('wisdom_expenses', JSON.stringify(expenses));
    if (databaseHydrated) {
      replaceDatabaseResource('expenses', expenses).catch(error => console.warn('Expense database sync failed.', error));
    }
  }, [expenses, databaseHydrated]);
  ...
  const addStudent = (studentData: Omit<Student, 'id'>): Student => {
    ...
    const newId = `std-${Date.now().toString(36)}-${randomSuffix}`;
